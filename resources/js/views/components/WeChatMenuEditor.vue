<template>
  <div class="warp">
    ...
    />
    <div class="adjust">
      <event-assign v-if="selectedButton" :readonly="readonly" ref="eventAssign" v-model="selectedButton" @destroy="destroyButton(selectedArrPath)" />
...
export default {
  ...
  computed: {
    selectedButton: {
      get() {
        return _.get(this.button, this.selectedStrPath) || null
      },
      set(button) {
        const arr = this.selectedArrPath

        if (arr.length === 1) {
          this.$set(this.button, arr[0], button)
        }

        if (arr.length === 2) {
          this.$set(this.button[arr[0]].sub_button, arr[1], button)
        }
      }
    }
  },
  watch: {
    ...
    button: {
      ...
      deep: true,
      handler(button) {
        !_.isEqual(button, this.value) && this.$emit('input', _.cloneDeep(button))
      }
    }
  },
  methods: {
    ...
    destroyButton(path) {
      // 父层级
      if (path.length === 1) {
        this.$delete(this.button, path[0])

        const newIndex = path[0]
        this.toggleSelectedArrPath(newIndex < this.button.length ? [newIndex] : [])
      }

      // 子层级
      if (path.length === 2) {
        this.$delete(this.button[path[0]].sub_button, [path[1]])

        const newSubIndex = path[1]
        this.toggleSelectedArrPath(newSubIndex < this.button[path[0]].sub_button.length ? [path[0], newSubIndex] : [path[0]])
      }
    },
    ...
    validate() {
      ...
      return deepValidate(this.button)
    }
  }
}
</script>
